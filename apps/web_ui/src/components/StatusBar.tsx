...
function buildWeatherTooltip(weather: WeatherCacheHealth | null): string {
  if (!weather) {
    return "HRRR solar history metrics are not available.";
  }
  const age = weather.age_seconds != null ? formatDuration(weather.age_seconds) : "unknown";
  const rows = weather.row_count ?? weather.file_count;
  const latest = weather.newest_valid_time ?? weather.latest_modified;
  const latestLabel = latest ? `Latest valid ${formatRelativeTime(latest)}` : "No valid solar history.";
  const location = weather.active_location
    ? ` - Active location ${weather.active_location.lat.toFixed(4)}, ${weather.active_location.lon.toFixed(4)}`
    : "";
  return `Rows: ${rows} - ${latestLabel} - Age ${age} - Size ${formatBytes(weather.bytes)}${location}`;
}
