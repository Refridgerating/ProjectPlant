...
import {
  ...
  createHubClient,
  type HrrrPointSnapshot as SdkHrrrSnapshot,
  type LocalWeatherSample,
  type WeatherLocation as SdkWeatherLocation,
  type WeatherLocationUpsertInput,
  type WeatherSeries as SdkWeatherSeries,
  type WeatherStation as SdkWeatherStation,
  type WateringAssumptions,
  type WateringClimateSummary,
  type WateringDiagnostics,
  type WateringOutputs,
  type WateringPlantProfile,
  type WateringPotMetrics,
  type WateringPotProfile,
  type WateringRecommendation,
  type WateringRequest,
  type WateringRequestSample,
} from "@projectplant/sdk/hub";
...
export type WeatherLocation = SdkWeatherLocation;
...
export type TelemetrySample = LocalWeatherSample & {
  timestamp: string;
...
export type WeatherStation = SdkWeatherStation;

export type WeatherSeries = Omit<SdkWeatherSeries, "samples"> & {
...
export type HrrrSnapshot = SdkHrrrSnapshot;
...
export { buildWateringRecommendationRequest };
...
export type { WateringRequest, WateringRequestSample };
...
function normalizeSdkHubError(error: unknown): never {
  if (error instanceof HubResponseError) {
    throw new HubApiError(error.message, error.status);
  }
  throw error;
}
...
export async function fetchHubInfo(signal?: AbortSignal): Promise<HubInfo> {
  const url = `${apiBase()}/info`;
  const response = await fetchWithConnectionTimeout(url, withUser({ signal }));
  if (!response.ok) {
...
export async function fetchManagedEffectiveAccess(signal?: AbortSignal): Promise<EffectiveAccessResponse> {
  const url = `${apiBase()}/me/effective-access`;
  const response = await fetchWithConnectionTimeout(url, withUser({ signal }));
  if (!response.ok) {
...
export async function fetchEventToken(signal?: AbortSignal): Promise<AuthTokenResponse> {
  ...
  if (!response.ok) {
    let detail = `Failed to obtain event token (${response.status})`;
    try {
      ...
      // ignore parse failures and use the status-based fallback
    }
...
export async function fetchWeatherCacheHealth(signal?: AbortSignal): Promise<WeatherCacheHealth> {
  const url = `${apiBase()}/health/weather_cache`;
  const response = await fetchWithConnectionTimeout(url, withUser({ signal }));
  if (!response.ok) {
    throw new Error(`Failed to load HRRR solar history health (${response.status})`);
  }
  return (await response.json()) as WeatherCacheHealth;
}

export async function fetchWeatherLocation(signal?: AbortSignal): Promise<WeatherLocation | null> {
  const init = withUser({ signal });
  try {
    return await createHubClient({ baseUrl: apiBase() }).fetchWeatherLocation({
      headers: init.headers,
      signal,
    });
  } catch (error) {
    throw normalizeSdkHubError(error);
  }
}

export async function upsertWeatherLocation(
  payload: WeatherLocationUpsertInput,
  signal?: AbortSignal
): Promise<WeatherLocation> {
  const init = withUser({ signal });
  try {
    return await createHubClient({ baseUrl: apiBase() }).upsertWeatherLocation(payload, {
      headers: init.headers,
      signal,
    });
  } catch (error) {
...
export async function fetchWeatherCacheEntries(
  ...
  if (!response.ok) {
    throw new Error(`Failed to load HRRR scratch file inventory (${response.status})`);
  }
...
export async function deleteWeatherCacheEntries(
  ...
  if (!response.ok) {
    throw new Error(`Failed to delete HRRR scratch files (${response.status})`);
  }
...
export async function storeWeatherCacheEntries(
  ...
  if (!response.ok) {
    throw new Error(`Failed to store HRRR scratch files (${response.status})`);
  }
...
export async function fetchStorageHealth(signal?: AbortSignal): Promise<StorageHealth> {
  const url = `${apiBase()}/health/storage`;
  const response = await fetchWithConnectionTimeout(url, withUser({ signal }));
  if (!response.ok) {
...
export async function fetchHealthEvents(
  ...
  const query = search.toString();
  const url = `${apiBase()}/health/events${query ? `?${query}` : ""}`;
  const response = await fetchWithConnectionTimeout(url, withUser({ signal }));
  if (!response.ok) {
...
export async function fetchLocalWeather(
  ...
  const init = withUser({ signal });
  try {
    const series = await createHubClient({ baseUrl: apiBase() }).fetchLocalWeather(params, {
      headers: init.headers,
      signal,
    });
    return { ...series, samples: series.samples as TelemetrySample[] };
  } catch (error) {
    throw normalizeSdkHubError(error);
  }
}
...
export async function fetchHrrrPoint(
  ...
  const init = withUser({ signal });
  try {
    return await createHubClient({ baseUrl: apiBase() }).fetchHrrrPoint(params, {
      headers: init.headers,
      signal,
    });
  } catch (error) {
...
export async function fetchWateringRecommendation(
  ...
  const init = withUser({ signal });
  const client = createHubClient({ baseUrl: apiBase() });
  try {
    return await client.fetchWateringRecommendation(payload, {
      headers: init.headers,
      signal,
    });
  } catch (error) {
