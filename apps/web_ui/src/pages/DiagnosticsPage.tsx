  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        />
        <DiagnosticCard
          title="HRRR Solar History"
          status={weather?.status ?? "unknown"}
          lines={[
            { label: "Rows", value: formatCacheFileCount(weather) },
            { label: "DB size", value: formatCacheSize(weather) },
            { label: "Active location", value: formatActiveWeatherLocation(weather) },
            {
              label: "Latest valid",
...
function cacheIsEmpty(weather: WeatherCacheHealth | null | undefined): boolean {
  ...
  const count = weather.row_count ?? weather.file_count;
  return weather.state === "empty" || count === 0;
}
...
function formatCacheFileCount(weather: WeatherCacheHealth | null): string {
  ...
  const count = weather.row_count ?? weather.file_count;
  return cacheIsEmpty(weather) ? "empty" : count.toString();
}
...
function formatCacheLastUpdate(weather: WeatherCacheHealth | null): string {
  ...
  return weather.newest_valid_time
    ? formatRelative(weather.newest_valid_time)
    : weather.latest_modified
      ? formatRelative(weather.latest_modified)
      : "unknown";
}

function formatActiveWeatherLocation(weather: WeatherCacheHealth | null): string {
  const location = weather?.active_location;
  if (!location) {
    return "n/a";
  }
  const label = `${location.lat.toFixed(4)}, ${location.lon.toFixed(4)}`;
  const source = weather?.location_source?.trim();
  if (!source) {
    return label;
  }
  return `${label} (${source.replace(/_/g, " ")})`;
}
