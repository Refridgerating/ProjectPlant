...
from device_protocol.topics import IRRIGATION_CMD_TOPIC_FMT, METRICS_TOPIC_FMT, PLANT_TELEMETRY_FILTER
        ...
        while self._running:
            try:
                async with self._client.messages() as messages:
                    await self._client.subscribe(PLANT_TELEMETRY_FILTER)
                    async for message in messages:
                        if not _topic_matches(message.topic, PLANT_TELEMETRY_FILTER):
                            continue
                ...
                try:
                    await self._client.unsubscribe(PLANT_TELEMETRY_FILTER)
