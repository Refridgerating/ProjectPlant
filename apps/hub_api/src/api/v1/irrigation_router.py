...
def _build_diagnostics(payload: IrrigationRequest, result: PenmanMonteithResult) -> DiagnosticsModel:
    notes = build_irrigation_diagnostic_notes(
        has_solar_radiation=any(sample.solar_radiation_w_m2 is not None for sample in payload.samples),
        has_wind_speed=any(sample.wind_speed_m_s is not None for sample in payload.samples),
        assumed_wind_speed_m_s=payload.assumed_wind_speed_m_s,
        daily_water_liters=result.outputs.daily_water_liters,
        max_event_liters=result.pot_metrics.max_event_liters,
    )
