...
class PumpStatusSnapshot:
    ...
    tz_offset_minutes_current: Optional[int] = None
    ...
    def to_dict(self) -> Dict[str, object]:
        ...
        if self.schedule_timezone_posix is not None:
            payload["scheduleTimezonePosix"] = self.schedule_timezone_posix
        if self.tz_offset_minutes_current is not None:
            payload["tzOffsetMinutesCurrent"] = self.tz_offset_minutes_current
        return payload
    ...
    def update(self, snapshot: PumpStatusSnapshot, *, merge: bool = False) -> None:
        ...
        if normalized_id != original_id:
            snapshot = PumpStatusSnapshot(
                ...
                schedule_timezone_posix=snapshot.schedule_timezone_posix,
                tz_offset_minutes_current=snapshot.tz_offset_minutes_current,
            )
        with self._lock:
            if merge:
                ...
                if existing is not None:
                    snapshot = PumpStatusSnapshot(
                        ...
                        schedule_timezone_posix=snapshot.schedule_timezone_posix
                        if snapshot.schedule_timezone_posix is not None
                        else existing.schedule_timezone_posix,
                        tz_offset_minutes_current=snapshot.tz_offset_minutes_current
                        if snapshot.tz_offset_minutes_current is not None
                        else existing.tz_offset_minutes_current,
                    )
            if normalized_id != original_id:
