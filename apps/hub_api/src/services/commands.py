...
from device_protocol.topics import COMMAND_TOPIC_FMT, SENSORS_TOPIC_FMT, STATUS_TOPIC_FMT
...
class CommandService:
    ...
    async def control_ic_zone1(
        ...
        *,
        ...
        overall_start = time.monotonic()
        zone_result = await self.send_ic_zone1_override(
            pot_id,
            zone_on=on,
            duration_ms=duration_int,
            timeout=timeout,
        ...
        if timeout is not None:
            ...
            if remaining <= 0:
                raise CommandTimeoutError(
                    f"Timed out waiting for sensor reading after ic zone 1 command for {pot_id}"
                )
            sensor_timeout = remaining

        sensor_result = await self.request_sensor_read(pot_id, timeout=sensor_timeout)
        payload = dict(sensor_result.payload)
        payload["requestId"] = zone_result.request_id
        return SensorReadResult(request_id=zone_result.request_id, payload=payload)
    ...
    async def _execute_command(
        ...
        *,
        ...
        payload_dict: dict[str, Any] = sensor_read_command(request_id=request_id) if command == "sensor_read" else {
            "requestId": request_id,
            "command": command,
        }
        if command_payload:
            payload_dict.update(command_payload)
        validate_device_command_payload(payload_dict)
        payload_json = encode_json_payload(payload_dict)
    ...
    async def send_pump_override(
        ...
        *,
        ...
        payload_dict = actuator_command("pump", request_id=request_id, on=pump_on, duration_ms=duration_ms)
        payload = encode_json_payload(payload_dict)
    ...
    async def send_ic_zone1_override(
        self,
        pot_id: str,
        *,
        zone_on: bool,
        ...
        payload_dict = actuator_command("icZone1", request_id=request_id, on=zone_on, duration_ms=duration_ms)
        payload = encode_json_payload(payload_dict)
        ...
        async with client.messages() as messages:
            ...
            try:
                ...
                raise CommandServiceError(f"Failed to publish ic zone 1 override command to {command_topic}") from exc
            ...
            try:
                while True:
                    ...
                    self._logger.debug(
                        "Received ic zone 1 status for %s in %.2f s", pot_id, time.monotonic() - start_monotonic
                    )
                    return CommandAckResult(request_id=request_id, payload=data)
            finally:
                try:
    ...
    async def send_fan_override(
        ...
        *,
        ...
        payload_dict = actuator_command("fan", request_id=request_id, on=fan_on, duration_ms=duration_ms)
        payload = encode_json_payload(payload_dict)
    ...
    async def send_mister_override(
        ...
        *,
        ...
        payload_dict = actuator_command("mister", request_id=request_id, on=mister_on, duration_ms=duration_ms)
        payload = encode_json_payload(payload_dict)
    ...
    async def send_light_override(
        ...
        *,
        ...
        payload_dict = actuator_command("light", request_id=request_id, on=light_on, duration_ms=duration_ms)
        payload = encode_json_payload(payload_dict)
    ...
    async def set_device_name(
        ...
        *,
        ...
        payload_dict = device_name_command(request_id=request_id, name=cleaned)
        payload = encode_json_payload(payload_dict)
    ...
    async def set_sensor_mode(
        ...
        *,
        ...
        payload_dict = sensor_mode_command(request_id=request_id, mode=normalized_mode)
        payload = encode_json_payload(payload_dict)
    ...
    async def set_device_schedule(
        ...
        *,
        ...
        schedule_timezone_posix: Optional[str] = None,
        schedule_updated_at_ms: Optional[int] = None,
        timeout: Optional[float] = None,
    ) -> CommandAckResult:
        pot_id = self._normalize_pot_id(pot_id)
        ...
        ic_zone1_payload = (
            dict(schedule.get("icZone1", {}))
            if isinstance(schedule.get("icZone1"), Mapping)
            else dict(schedule.get("ic_zone1", {}))
            if isinstance(schedule.get("ic_zone1"), Mapping)
            else None
        )
        if ic_zone1_payload:
            schedule_payload["icZone1"] = ic_zone1_payload
        payload_dict = schedule_command(
            request_id=request_id,
            schedule=schedule_payload,
            tz_offset_minutes=tz_offset_minutes,
            schedule_timezone_posix=schedule_timezone_posix,
            schedule_updated_at_ms=schedule_updated_at_ms,
        )
        payload = encode_json_payload(payload_dict)
        start_monotonic = time.monotonic()
