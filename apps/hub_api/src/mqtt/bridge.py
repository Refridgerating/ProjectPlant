...
class MqttBridge:
    ...
    async def start(self) -> None:
        if self._started:
            return
        self._started = True
        self._logger.info("Starting MQTT bridge for topics: %s", ", ".join(BRIDGE_SUBSCRIPTIONS))
    ...
    async def _handle_firmware_message(self, message: Message) -> None:
        ...
        try:
            validate_device_sensor_payload(telemetry_payload)
        except ProtocolPayloadError as exc:
            self._logger.debug("Normalized sensor payload for %s failed protocol validation: %s", pot_id, exc)
            return
        ...
        await event_bus.publish("telemetry", {"category": "pot", "sample": telemetry_payload})

        payload_json = json.dumps(telemetry_payload, separators=(",", ":"))
    ...
    async def _handle_status_message(self, message: Message) -> None:
        ...
        try:
            validate_device_status_payload(snapshot_payload)
        except ProtocolPayloadError as exc:
            self._logger.debug("Normalized status payload for %s failed protocol validation: %s", pot_id, exc)
            return

        pump_status_cache.update(snapshot)
        await event_bus.publish("status", snapshot_payload)
        payload_json = json.dumps(snapshot_payload, separators=(",", ":"))
...
def build_status_payload_from_data(data: dict[str, Any], pot_id: str) -> Optional[PumpStatusSnapshot]:
    ...
    if sensor_mode is None:
        sensor_mode = _coerce_str(data.get("sensor_mode"))
    schedule_timezone_posix = _coerce_str(data.get("scheduleTimezonePosix"))
    if schedule_timezone_posix is None:
        schedule_timezone_posix = _coerce_str(data.get("schedule_timezone_posix"))
    tz_offset_minutes_current_value = _coerce_float(data.get("tzOffsetMinutesCurrent"))
    if tz_offset_minutes_current_value is None:
        tz_offset_minutes_current_value = _coerce_float(data.get("tz_offset_minutes_current"))
    tz_offset_minutes_current = int(tz_offset_minutes_current_value) if tz_offset_minutes_current_value is not None else None
    ...
    if (
        ...
        and schedule_timezone_posix is None
        and tz_offset_minutes_current is None
    ...
    return PumpStatusSnapshot(
        ...
        tz_offset_minutes_current=tz_offset_minutes_current,
    )
