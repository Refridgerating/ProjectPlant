...
def _resolve_hub_path(raw_value: object) -> str | None:
    ...
    if normalized.startswith("apps/hub_api/"):
...
class Settings(BaseSettings):
    # Load apps/hub_api/.env and accept env keys in any case
    ...
    apple_oauth_client_ids: Annotated[List[str], NoDecode] = Field(
        default_factory=list,
    ...
    hrrr_cache_dir: str = Field(
        default="data/hrrr/cache",
        description="Directory for cached HRRR GRIB2 downloads (relative to the hub app root).",
    )
    hrrr_archive_dir: str = Field(
        default="data/hrrr/archive",
        description="Directory where archived HRRR assets will be stored (relative to the hub app root).",
    )
    hrrr_solar_history_db: str = Field(
        default="data/hrrr/solar_history.sqlite",
        description="SQLite database storing extracted HRRR solar history.",
    )
    hrrr_active_location_path: str = Field(
        default="data/hrrr/active_location.json",
        description="JSON file storing the active hub weather location used for HRRR defaults.",
    )
    hrrr_solar_retention_hours: int = Field(
        default=72,
        ge=1,
        le=240,
        description="Retention window for extracted HRRR solar history.",
    )
    ...
    alerts_email_to: Annotated[List[str], NoDecode] = Field(
        default_factory=list,
    ...
    plant_schedule_tz_posix: str | None = Field(
        default=None,
        description="Optional POSIX TZ string used for plant schedule sync when Linux timezone auto-detection is unavailable.",
    )

    @field_validator("cors_origins", mode="before")
    @classmethod
    def normalize_cors(cls, v):
        normalized = _normalize_string_list(v, auto_http=True)
    ...
    @field_validator("alerts_email_to", mode="before")
    @classmethod
    def normalize_alert_recipients(cls, value):
        return _normalize_string_list(value)

    @field_validator("plant_schedule_tz_posix", mode="before")
    @classmethod
    def normalize_plant_schedule_tz_posix(cls, value):
        if value is None:
            return None
    ...
    @field_validator(
        "hrrr_cache_dir",
        "hrrr_archive_dir",
        "hrrr_solar_history_db",
        "hrrr_active_location_path",
        "pot_telemetry_db",
        "provision_event_log",
        "alerts_event_log",
    ...
    def resolve_runtime_paths(cls, value):
        return _resolve_hub_path(value)
