...
from device_protocol.payloads import validate_device_command_payload
...
def _decode_command_payload(payload: str) -> dict:
    data = json.loads(payload)
    assert validate_device_command_payload(data)
    return data
...
class FakeClient(_BaseFakeClient):
    ...
    async def publish(self, topic: str, payload: str, qos: int = 0, retain: bool = False) -> None:
        ...
        data = _decode_command_payload(payload)
...
class SilentPumpStatusClient(PumpStatusClient):
    async def publish(self, topic: str, payload: str, qos: int = 0, retain: bool = False) -> None:
        self.published.append((topic, payload, qos, retain))
        self.request_ids.append(_decode_command_payload(payload)["requestId"])
...
async def test_request_sensor_read_publishes_command_and_returns_fresh_payload(monkeypatch):
    ...
    published_data = _decode_command_payload(payload)
...
async def test_set_device_schedule_publishes_schedule_and_waits_for_status(monkeypatch):
    ...
    assert published_data["scheduleTimezonePosix"] == "EST5EDT,M3.2.0/2,M11.1.0/2"
