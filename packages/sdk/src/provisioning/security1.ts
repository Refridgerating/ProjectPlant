...
class AesCtrStream {
  ...
  static async create(keyBytes: Uint8Array, initialCounter: Uint8Array): Promise<AesCtrStream> {
    ...
    const key = await cryptoApi.subtle.importKey("raw", toArrayBuffer(keyBytes), { name: "AES-CTR" }, false, ["encrypt"]);
  ...
  private async refillBlock(): Promise<void> {
    ...
    const encrypted = await cryptoApi.subtle.encrypt(
      {
        name: "AES-CTR",
        counter: toArrayBuffer(this.counter),
        length: 128,
      },
      this.key,
      new ArrayBuffer(16)
...
export class Security1Session {
  ...
  static async create(popValue: string): Promise<Security1Session> {
    ...
    try {
      keyPair = (await cryptoApi.subtle.generateKey({ name: "X25519" }, true, ["deriveBits"])) as CryptoKeyPair;
    } catch (error) {
      throw new Error(
        `X25519 is unavailable in this runtime: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  ...
  async initialise(devicePublicKey: Uint8Array, deviceRandom: Uint8Array): Promise<void> {
    ...
    if (trimmedPop.length > 0) {
      const digest = new Uint8Array(
        await cryptoApi.subtle.digest("SHA-256", toArrayBuffer(new TextEncoder().encode(trimmedPop)))
      );
