import { BleBridge, type BleDevice } from "@projectplant/native-bridge/ble";
import {
...
export class EspBleProvisioner {
  ...
  async getProtocolInfo(): Promise<ProtocolInfo> {
    ...
    try {
      const parsed = JSON.parse(raw) as { prov?: { ver?: string; cap?: string[] } };
  ...
  async scanWifiNetworks(): Promise<WiFiScanEntry[]> {
    ...
    const startStatus = decodeWiFiScanStartResponse(await this.callEndpoint("prov-scan", encodeWiFiScanStart(), true));
    if (startStatus !== 0) {
    ...
    while (offset < resultCount) {
      ...
      const result = decodeWiFiScanResultResponse(
        await this.callEndpoint("prov-scan", encodeWiFiScanResultRequest(offset, count), true)
      );
      if (result.status !== 0) {
  ...
  async sendHubConfig(payload: HubConfigPayload): Promise<HubConfigResponse | null> {
    ...
    const response = await this.callEndpoint("hub", new TextEncoder().encode(JSON.stringify(body)), true);
  ...
  async fetchWiFiStatus(): Promise<WiFiStatus> {
    return decodeWiFiConfigGetStatusResponse(await this.callEndpoint("prov-config", encodeWiFiConfigGetStatus(), true));
  }

  async waitForWifiConnection(options: WaitForWifiOptions): Promise<WaitForWifiResult> {
    const timeoutMs = Math.max(options.timeoutMs, 1000);
    const intervalMs = Math.max(options.intervalMs ?? 2500, 500);
  ...
  private async callEndpoint(endpointName: EndpointName, payload: Uint8Array, encrypted: boolean): Promise<Uint8Array> {
  ...
  private async discoverEndpointMap(): Promise<void> {
    ...
    for (const service of services) {
      ...
      for (const characteristic of service.characteristics) {
        const descriptor = characteristic.descriptors.find((entry) => entry.uuid.toLowerCase() === USER_DESCRIPTION_UUID);
        if (!descriptor) {
          continue;
        }
        try {
    ...
    if (!this.hasAllRequiredEndpoints()) {
      ...
      throw new Error(`Unable to discover required provisioning endpoints. Found: ${found}`);
    }
  }
